...
TEST_CASE("OrderBook - Best Bid/Ask Methods", "[tests]") {
    ...
    orderBook.submitOrder(Order(Side::BUY, orderBook.nextOrderId(), 1.75, 50));
    REQUIRE(orderBook.getBestBidOrder()->getPrice() == Approx(1.75).epsilon(EPSILON));
    REQUIRE(orderBook.getBestAskOrder() == nullptr);

    orderBook.submitOrder(Order(Side::BUY, orderBook.nextOrderId(), 1.85, 50));
    REQUIRE(orderBook.getBestBidOrder()->getPrice() == Approx(1.85).epsilon(EPSILON));
    REQUIRE(orderBook.getBestAskOrder() == nullptr);

    orderBook.submitOrder(Order(Side::BUY, orderBook.nextOrderId(), 1.65, 50));
    REQUIRE(orderBook.getBestBidOrder()->getPrice() == Approx(1.85).epsilon(EPSILON));
    REQUIRE(orderBook.getBestAskOrder() == nullptr);

    orderBook.clear();

    orderBook.submitOrder(Order(Side::SELL, orderBook.nextOrderId(), 1.65, 50));
    REQUIRE(orderBook.getBestAskOrder()->getPrice() == Approx(1.65).epsilon(EPSILON));
    REQUIRE(orderBook.getBestBidOrder() == nullptr);

    orderBook.submitOrder(Order(Side::SELL, orderBook.nextOrderId(), 1.55, 50));
    REQUIRE(orderBook.getBestAskOrder()->getPrice() == Approx(1.55).epsilon(EPSILON));
    REQUIRE(orderBook.getBestBidOrder() == nullptr);

    orderBook.submitOrder(Order(Side::SELL, orderBook.nextOrderId(), 1.75, 50));
    REQUIRE(orderBook.getBestAskOrder()->getPrice() == Approx(1.55).epsilon(EPSILON));
    REQUIRE(orderBook.getBestBidOrder() == nullptr);
}
...
TEST_CASE("MarketOrder - Execution Tests", "[tests]") {
    ...
    orderBook.submitOrder(Order(Side::SELL, limOrderId, 1.75, 100));
    Order marketOrder1(Side::BUY, orderBook.nextOrderId(), 100);
    orderBook.submitOrder(marketOrder1);
